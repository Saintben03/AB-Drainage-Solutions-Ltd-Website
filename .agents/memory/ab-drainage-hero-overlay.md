...
- MOBILE (fencing formula): `min-h-[92vh] items-end pb-24`, bright image `saturate-[1.35] brightness-[1.08] contrast-[1.1]`, slow pan via `.animate-heropan-mobile` (index.css, scoped `@media (max-width:767px)`, in reduced-motion block), overlays = bottom `from-black/80` + left `from-black/55` gradients + `#01618E`/40 `mixBlendMode:color` cast. NO heavy blue tint on mobile.
