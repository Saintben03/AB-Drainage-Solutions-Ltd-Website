...
**Three forms now really send via Web3Forms** (as of Jul 2026): ContactForm, EmergencyModal, and FlexiPayForm (Services page — was previously a mailto: handoff that silently failed without a mail app). No `cc`/`ccemail` field is sent (Pro-only). Remaining mailto: links on ABGroup* pages are unaffected (they open the visitor's mail client).
