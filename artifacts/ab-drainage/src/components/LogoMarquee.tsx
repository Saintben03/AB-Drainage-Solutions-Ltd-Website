...
export function LogoMarquee({ logos, fadeColor = "#e0f2fe" }: { logos: MarqueeLogo[]; fadeColor?: string }) {
  return (
    <>
      {/* Mobile: static, centered, evenly spaced row.
          The animated marquee below relies on translateX over a width:max-content
          flex track, which mis-computes on iOS Safari (intrinsic image sizing during
          async decode) and produces misaligned gaps. A static row is bulletproof. */}
      <div className="flex md:hidden flex-wrap items-center justify-center gap-x-10 gap-y-6 px-4">
        {logos.map((logo, i) => (
          <img
            key={i}
            src={logo.src}
            ...
            loading="eager"
            decoding="async"
            className={`w-auto object-contain drop-shadow-[0_6px_14px_rgba(15,23,42,0.18)] ${logo.className ?? "h-10"}`}
          />
        ))}
      </div>
